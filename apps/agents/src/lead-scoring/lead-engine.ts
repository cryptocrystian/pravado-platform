...
/**
 * Lead Scoring Engine
 * Manages lead qualification, scoring, and pipeline management
 */
export class LeadScoringEngine extends EventEmitter {
  // =====================================================
  // SCORE CALCULATION
  // =====================================================
   ...
   */
  async calculateLeadScore(input: RecalculateLeadScoreInput): Promise<LeadScore> {
   ...
   * Update lead stage
   */
  async updateStage(input: UpdateLeadStageInput): Promise<boolean> {
   ...
   */
  async autoQualify(input: AutoQualifyInput): Promise<boolean> {
    const threshold = input.threshold || 70;

    const leadScore = await this.getLeadScore(
      input.contactId,
      input.campaignId,
  ...
  /**
   * Get lead trends for a contact
   */
  async getLeadTrends(contactId: string, organizationId: string): Promise<LeadTrendResult> {
    ...
    const trends: LeadScoreTrendPoint[] = (data || []).map((row: any) => ({
  ...
  /**
   * Summarize lead performance with GPT
   */
  async summarizeLeadPerformance(
    input: SummarizeLeadPerformanceInput
      ...
      messages: [
        {
          role: 'system',
          content:
            'You are an expert sales analyst evaluating lead quality and providing actionable insights.',
        },
        {
          role: 'user',
          content: prompt,
  ...
  async getTopLeads(
    ...
    const leads = (data || []).map((d) => this.mapLeadScoreFromDb(d)) as EnrichedLeadScore[];
  ...
  async getQualifiedLeads(
    ...
    // Recently qualified (last 7 days)
    const sevenDaysAgo = new Date();
    sevenDaysAgo.setDate(sevenDaysAgo.getDate() - 7);

    const recentlyQualified = leads.filter(
      (l) => new Date(l.lastStageChangeAt) > sevenDaysAgo
    );
   ...
   */
  async getDisqualifiedLeads(
    organizationId: string,
    campaignId?: string
  ): Promise<DisqualifiedLeadsResult> {
    let query = supabase
      .from('lead_scores')
      .select('*')
      .eq('organization_id', organizationId)
  ...
  private mapLeadScoreFromDb(data: any): LeadScore {
    return {
      id: data.id,
      ...
      ragStatus: data.rag_status as RAGStatus,
