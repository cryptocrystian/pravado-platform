...
exports.competitiveIntelEngine = exports.CompetitiveIntelEngine = void 0;
...
class CompetitiveIntelEngine extends events_1.EventEmitter {
    constructor() {
        super();
    }
    ...
    async updateCompetitor(input) {
        try {
            ...
            if (input.headquarters !== undefined)
                updateData.headquarters = input.headquarters;
            if (input.foundedYear !== undefined)
                updateData.founded_year = input.foundedYear;
            if (input.employeeCount !== undefined)
                updateData.employee_count = input.employeeCount;
            if (input.fundingStage !== undefined)
                updateData.funding_stage = input.fundingStage;
            if (input.totalFundingUsd !== undefined)
                updateData.total_funding_usd = input.totalFundingUsd;
            if (input.primaryProduct !== undefined)
                updateData.primary_product = input.primaryProduct;
            if (input.productCategories !== undefined)
                updateData.product_categories = input.productCategories;
            if (input.targetMarket !== undefined)
                updateData.target_market = input.targetMarket;
            ...
            if (input.facebookUrl !== undefined)
                updateData.facebook_url = input.facebookUrl;
    ...
    async getCompetitorProfile(organizationId, competitorId) {
        try {
    ...
    async updateIntelEvent(input) {
        try {
            ...
            if (input.impactScore !== undefined)
                updateData.impact_score = input.impactScore;
            ...
            if (input.actionNotes !== undefined)
                updateData.action_notes = input.actionNotes;
            if (input.aiSummary !== undefined)
                updateData.ai_summary = input.aiSummary;
            if (input.keyInsights !== undefined)
                updateData.key_insights = input.keyInsights;
            if (input.affectedCampaigns !== undefined)
                updateData.affected_campaigns = input.affectedCampaigns;
            if (input.isVerified !== undefined)
                updateData.is_verified = input.isVerified;
            const { data, error } = await supabase
                .from('intel_events')
                .update(updateData)
                .eq('id', input.eventId)
    ...
    async getCompetitorMetrics(organizationId, competitorId) {
        try {
    ...
    async getMarketTrends(organizationId, category, periodStart, periodEnd) {
        try {
...
Format as JSON with keys: positioningSummary, strengths (array), weaknesses (array), threatsToUs (array), opportunities (array), strategicRecommendations (array), keyTakeaways (array)`;
            const completion = await openai.chat.completions.create({
                ...
                messages: [
                    {
                        role: 'system',
                        content: 'You are a competitive intelligence analyst specializing in strategic competitor analysis. Provide actionable insights in JSON format.',
                    },
                    {
                        role: 'user',
                        content: prompt,
    ...
    async summarizeMarketTrends(input) {
        try {
            const marketSummary = await this.getMarketTrends(input.organizationId, input.category, input.periodStart, input.periodEnd);
...
Format as JSON with keys: summary, categoryMomentum, keyTrends (array), threats (array), opportunities (array), strategicRecommendations (array), topMovers (array with competitorName and activity fields)`;
            const completion = await openai.chat.completions.create({
                ...
                messages: [
                    {
                        role: 'system',
                        content: 'You are a market intelligence analyst specializing in competitive landscape analysis. Provide actionable insights in JSON format.',
                    },
                    {
                        role: 'user',
                        content: prompt,
...
exports.CompetitiveIntelEngine = CompetitiveIntelEngine;
exports.competitiveIntelEngine = new CompetitiveIntelEngine();
//# sourceMappingURL=competitive-engine.js.map