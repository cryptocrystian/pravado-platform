// =====================================================
// COLLABORATION ENGINE - Advanced Collaboration & Handoff System
// =====================================================
...
import type {
  ...
  UpdateCollaborationThreadInput,
...
export class CollaborationEngine extends EventEmitter {
   ...
   * Request a handoff
   */
  async requestHandoff(input: Omit<CreateHandoffRequestInput, 'organizationId'> & { fromUserId: string; organizationId: string }): Promise<HandoffRequest> {
    const { fromUserId, organizationId, ...rest } = input;

    // Create handoff request via database function
   ...
   * Accept a handoff
   */
  async acceptHandoff(input: AcceptHandoffInput): Promise<boolean> {
    ...
    // Get request details for timeline logging
   ...
   * Decline a handoff
   */
  async declineHandoff(input: DeclineHandoffInput): Promise<boolean> {
   ...
   */
  async getUserHandoffQueue(userId: string, organizationId: string): Promise<EnrichedHandoffRequest[]> {
   ...
   */
  async createThread(input: Omit<CreateCollaborationThreadInput, 'organizationId'> & { userId: string; organizationId: string }): Promise<CollaborationThread> {
   ...
   */
  async addComment(input: Omit<CreateCollaborationCommentInput, 'organizationId'> & { authorId: string; organizationId: string }): Promise<CollaborationComment> {
   ...
   */
  private async mentionUser(threadId: string, mentioningUserId: string, targetUserId: string, organizationId: string): Promise<void> {
    // Emit mention event
  ...
  /**
   * Get campaign collaboration context
   */
  async getCampaignDiscussion(campaignId: string, userId: string, organizationId: string): Promise<CampaignCollaborationContext> {
    ...
    return {
      threads: (context.threads || []).map((t: any) => this.mapThread(t)),
      comments: context.comments || {},
      totalThreads: (context.threads || []).length,
      totalComments: Object.values(context.comments || {}).reduce((sum: number, comments: any) => sum + comments.length, 0),
    };
  }

  /**
   * Summarize a thread using GPT-4
   */
  async summarizeThread(input: SummarizeThreadInput): Promise<ThreadSummary> {
   ...
   */
  private buildSummaryPrompt(thread: any, comments: any[], input: SummarizeThreadInput): string {
  ...
  private mapHandoffRequest(row: any): HandoffRequest {
    return {
      id: row.id,
      campaignId: row.campaign_id,
      fromUserId: row.from_user_id,
  ...
  private mapThread(row: any): CollaborationThread {
    return {
      id: row.id,
  ...
  private mapComment(row: any): CollaborationComment {
    return {
      id: row.id,
