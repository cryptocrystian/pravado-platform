...
exports.reportEngine = exports.ReportEngine = void 0;
...
class ReportEngine extends events_1.EventEmitter {
    async generateCampaignReport(input) {
        try {
            const { data: reportData, error: createError } = await supabase.rpc('generate_campaign_report', {
            ...
            try {
                const metrics = await this.getMetricsSnapshot(input.campaignId, input.organizationId, input.periodStart, input.periodEnd);
                ...
                const keyFindings = this.extractKeyFindings(executiveSummary, performanceAnalysis, sentimentAnalysis);
                const recommendations = this.extractRecommendations(performanceAnalysis, agentEffectiveness, strategicInsights);
                const sections = await this.buildReportSections(executiveSummary, performanceAnalysis, agentEffectiveness, sentimentAnalysis, strategicInsights, metrics);
    ...
    async getMetricsSnapshot(campaignId, organizationId, periodStart, periodEnd) {
    ...
    async generateExecutiveSummary(metrics, input) {
        ...
        const completion = await openai.chat.completions.create({
            ...
            messages: [
                {
                    role: 'system',
                    content: 'You are an expert PR and marketing analyst creating executive summaries for campaign performance reports. Be concise, data-driven, and actionable.',
                },
                {
                    role: 'user',
                    content: prompt,
    ...
    async summarizePerformanceTrends(metrics, input) {
        ...
        const completion = await openai.chat.completions.create({
            ...
            messages: [
                {
                    role: 'system',
                    content: 'You are an expert data analyst evaluating campaign performance trends. Identify patterns, correlations, and actionable insights from the data.',
                },
                {
                    role: 'user',
                    content: prompt,
    ...
    async summarizeAgentEffectiveness(metrics, input) {
        ...
        const completion = await openai.chat.completions.create({
            ...
            messages: [
                {
                    role: 'system',
                    content: 'You are an expert in AI agent performance evaluation. Analyze agent efficiency, success patterns, and optimization opportunities.',
                },
                {
                    role: 'user',
                    content: prompt,
    ...
    async summarizeSentimentTrends(metrics, input) {
        ...
        const completion = await openai.chat.completions.create({
            ...
            messages: [
                {
                    role: 'system',
                    content: 'You are an expert in sentiment analysis and relationship management. Evaluate contact sentiment patterns and their implications.',
                },
                {
                    role: 'user',
                    content: prompt,
    ...
    async generateStrategyInsights(metrics, input) {
        ...
        const completion = await openai.chat.completions.create({
            ...
            messages: [
                {
                    role: 'system',
                    content: 'You are a strategic PR consultant providing high-level insights and recommendations. Focus on opportunities, risks, and strategic optimizations.',
                },
                {
                    role: 'user',
                    content: prompt,
    ...
    buildExecutiveSummaryPrompt(metrics) {
    ...
    buildPerformanceTrendsPrompt(metrics) {
    ...
    buildAgentEffectivenessPrompt(metrics) {
    ...
    buildSentimentTrendsPrompt(metrics) {
    ...
    buildStrategyInsightsPrompt(metrics) {
    ...
    async buildReportSections(executiveSummary, performanceAnalysis, agentEffectiveness, sentimentAnalysis, strategicInsights, metrics) {
        const sections = [];
        sections.push({
    ...
    extractKeyFindings(executiveSummary, performanceAnalysis, sentimentAnalysis) {
        const findings = [];
    ...
    extractRecommendations(performanceAnalysis, agentEffectiveness, strategicInsights) {
    ...
    async getLatestCampaignReport(campaignId, organizationId) {
        const { data, error } = await supabase
            .from('generated_reports')
            .select('*')
    ...
    async retryReport(reportId, organizationId) {
...
exports.reportEngine = new ReportEngine();
