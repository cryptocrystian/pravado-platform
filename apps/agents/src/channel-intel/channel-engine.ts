...
/**
 * Channel Intelligence Engine
 * Manages channel effectiveness, sentiment analysis, and outreach optimization
 */
export class ChannelEngine extends EventEmitter {
  ...
  /**
   * Log an engagement with automatic sentiment analysis
   */
  async logEngagement(input: LogEngagementInput): Promise<string> {
    // Analyze sentiment if message provided and sentiment not set
    ...
    // Use the database function to log
   ...
   */
  async analyzeSentiment(input: AnalyzeSentimentInput): Promise<SentimentAnalysisResult> {
      ...
      messages: [
        {
          role: 'system',
          content:
            'You are an expert at analyzing communication sentiment and tone. Classify messages accurately and objectively.',
        },
        {
          role: 'user',
          content: prompt,
  ...
  private buildSentimentPrompt(input: AnalyzeSentimentInput): string {
  ...
  /**
   * Get best channel for a contact
   */
  async getBestChannelForContact(
    input: GetChannelRecommendationsInput
    ...
    for (const row of data || []) {
      ...
      const timeRec = timeData?.find((t: any) => t.channel === row.channel);

      // Determine recommended tone based on sentiment history
      const tone = await this.recommendTone(
        input.contactId,
        row.channel,
  ...
  /**
   * Recommend tone for contact
   */
  private async recommendTone(
    contactId: string,
    channel: ChannelType,
    organizationId: string
  ): Promise<'friendly' | 'professional' | 'assertive' | 'consultative'> {
    ...
    if (!data || data.length === 0) {
      return 'professional'; // Default for no data
    }
    ...
    // Logic for tone recommendation
    if (positiveCount >= 3 && hasReplies) {
      return 'friendly'; // They're engaged and positive
    } else if (negativeCount >= 2) {
      return 'professional'; // Keep it professional if negative signals
    } else if (hasReplies && positiveCount >= 1) {
      return 'consultative'; // They're responsive, be collaborative
    } else {
   ...
   */
  async getContactChannelProfile(
    contactId: string,
    organizationId: string
  ): Promise<ContactChannelProfile> {
    // Get performances
    ...
    // Calculate overall receptiveness (average across all channels)
    const avgReceptiveness =
      performances.reduce((sum, p) => sum + p.contactReceptivenessScore, 0) /
        (performances.length || 1);

    // Determine preferred channel (highest receptiveness)
    const preferredChannel = performances.sort(
      (a, b) => b.contactReceptivenessScore - a.contactReceptivenessScore
    )[0]?.channelType;
   ...
   */
  async getCampaignChannelStats(
    campaignId: string,
    organizationId: string
  ): Promise<CampaignChannelStats> {
    // Get performances
    const { data: perfData } = await supabase
      .from('channel_performance_aggregates')
      .select('*')
    ...
    const avgSentimentScore =
      performances.reduce((sum, p) => sum + p.avgSentimentScore, 0) / (performances.length || 1);
    ...
    const topContacts = await Promise.all(
      (topContactsData || []).map(async (item: any) => {
  ...
  /**
   * Summarize sentiment trends with GPT
   */
  async summarizeSentimentTrends(
    input: SummarizeSentimentTrendsInput
  ): Promise<SentimentTrendSummary> {
    const trends = await this.getSentimentTrends(
      input.contactId,
      input.organizationId,
      input.channelType
    );
      ...
      messages: [
        {
          role: 'system',
          content:
            'You are an expert at analyzing communication patterns and sentiment trends over time.',
        },
        {
          role: 'user',
          content: prompt,
    ...
    const avgSentimentByChannel = Object.entries(avgByChannel).reduce((acc, [channel, data]) => {
      acc[channel as ChannelType] = data.sum / data.count;
      return acc;
    ...
    const mostEngagedChannel = Object.entries(engagementsByChannel).sort(
      ([, a], [, b]) => b - a
    )[0]?.[0] as ChannelType | undefined;
  ...
  private formatTimeRationale(hour: number, dayOfWeek: number): string {
  ...
  private calculateSentimentTrend(
    engagements: EnrichedChannelEngagement[]
    ...
    const recentAvg =
      recent.reduce((sum, e) => sum + e.engagementScore, 0) / (recent.length || 1);
  ...
  private mapEngagementFromDb(data: any): EnrichedChannelEngagement {
    return {
      id: data.id,
  ...
  private mapPerformanceFromDb(data: any): ChannelPerformance {
    return {
      id: data.id,
      organizationId: data.organization_id,
      contactId: data.contact_id,
