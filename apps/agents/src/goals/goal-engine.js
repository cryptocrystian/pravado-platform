...
class GoalEngine extends events_1.EventEmitter {
    ...
    async updateGoal(input) {
        ...
        if (input.targetMetric !== undefined)
            updates.target_metric = input.targetMetric;
        if (input.priority !== undefined)
            updates.priority = input.priority;
        if (input.trackingMethod !== undefined)
            updates.tracking_method = input.trackingMethod;
        if (input.successConditions !== undefined)
            updates.success_conditions = input.successConditions;
    ...
    async getCampaignGoals(campaignId, organizationId) {
        ...
        return (data || []).map((row) => this.mapGoalSummaryFromDb(row));
    }
    ...
    async calculateGoalProgress(input) {
    ...
    async getAttributionMap(campaignId, organizationId) {
        ...
        for (const goal of goals) {
            ...
            const events = (eventsData || []).map((row) => this.mapAttributionEventFromDb(row));
    ...
    async getCampaignGoalsOverview(campaignId, organizationId) {
        const goals = await this.getCampaignGoals(campaignId, organizationId);
    ...
    async summarizeGoalPerformance(input) {
        let goals;
        let attributionMaps = [];
        if (input.goalId) {
        ...
        const completion = await openai.chat.completions.create({
            ...
            messages: [
                {
                    role: 'system',
                    content: 'You are a strategic analyst helping evaluate campaign goal performance. Provide concise, actionable insights.',
                },
                {
                    role: 'user',
                    content: prompt,
        ...
        const overallScore = goals.reduce((sum, g) => sum + g.completionScore, 0) / (goals.length || 1);
    ...
    buildGoalSummaryPrompt(goals, attributionMaps, input) {
    ...
    calculateVelocityTrend(goals) {
        const averageCompletion = goals.reduce((sum, g) => sum + g.completionScore, 0) / goals.length;
        const recentlyActive = goals.filter((g) => g.lastEventAt && new Date(g.lastEventAt) > new Date(Date.now() - 7 * 24 * 60 * 60 * 1000));
        const stalledGoals = goals.filter((g) => !g.lastEventAt ||
    ...
    buildCriticalGoalsSummary(criticalGoals) {
    ...
    buildStrategicDirective(activeGoals) {
    ...
    enrichGoalWithOutcome(goal, outcome) {
        const now = new Date();
    ...
    mapAttributionEventFromDb(data) {
        return {
            id: data.event_id,
            organizationId: '',
