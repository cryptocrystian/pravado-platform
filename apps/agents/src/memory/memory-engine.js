...
class MemoryEngine {
    async storeMemory(input) {
        try {
            ...
            const { data, error } = await supabase_1.supabase.rpc('store_agent_memory', {
    ...
    async getRelevantMemories(input) {
        try {
            const { data, error } = await supabase_1.supabase.rpc('query_relevant_memories', {
    ...
    async loadCampaignContext(campaignId, organizationId, limit = 20) {
        try {
            const { data, error } = await supabase_1.supabase.rpc('get_campaign_memory_context', {
    ...
    async summarizeMemories(request) {
        try {
            const memoryTexts = request.memories.map((m, i) => `[${i + 1}] ${m.memoryType}: ${m.content}`);
  ...
  "recommendations": ["...", "..."]
            ...
            const completion = await openai.chat.completions.create({
                ...
                messages: [
                    {
                        role: 'system',
                        content: 'You are an AI assistant analyzing agent memories to extract insights and patterns.',
                    },
    ...
    async insertKGNode(input) {
        try {
            let embedding = input.embedding;
            if (!embedding && input.description) {
                embedding = await this.generateEmbedding(`${input.label} ${input.description}`);
            }
            const { data, error } = await supabase_1.supabase.rpc('insert_knowledge_graph_node', {
    ...
    async linkKGNodes(input) {
        try {
            const { data, error } = await supabase_1.supabase.rpc('link_knowledge_graph_nodes', {
    ...
    async getKnowledgeGraphContext(campaignId, organizationId, nodeTypes, minImportance = 0.5) {
        try {
            const { data, error } = await supabase_1.supabase.rpc('get_graph_context_for_campaign', {
                p_campaign_id: campaignId,
                p_organization_id: organizationId,
    ...
    async injectContext(input) {
        try {
            ...
            if (input.queryText) {
                ...
                const context = await this.loadCampaignContext(input.campaignId, input.organizationId, input.maxMemories || 10);
            ...
            if (input.includeKnowledgeGraph && input.campaignId) {
                knowledgeGraph = await this.getKnowledgeGraphContext(input.campaignId, input.organizationId);
            }
    ...
    buildContextPrompt(memories, knowledgeGraph) {
        let prompt = '';
    ...
    async linkToKnowledgeGraph(memory, organizationId) {
        try {
