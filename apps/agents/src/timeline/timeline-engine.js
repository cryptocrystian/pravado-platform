...
exports.timelineEngine = exports.TimelineEngine = void 0;
...
class TimelineEngine {
    ...
    shouldGenerateAISummary(eventType) {
        const aiSummaryTypes = [
            'INSIGHT_GENERATED',
            'DECISION_MADE',
            'AGENT_RUN',
            'REVIEW_SUBMITTED',
    ...
    async summarizeEvent(input) {
        ...
        const completion = await openai.chat.completions.create({
            ...
            messages: [
                {
                    role: 'system',
                    content: 'You are an expert at creating concise, clear summaries of campaign events. Create a 1-2 sentence summary that captures the key information. Also provide a brief details section (2-3 sentences) and an importance score from 0.0 to 1.0.',
                },
                {
                    role: 'user',
                    content: prompt,
    ...
    async getCampaignTimeline(input) {
        ...
        return (data || []).map((row) => this.mapTimelineEvent(row));
    }
    async getGlobalTimeline(input) {
        ...
        return (data || []).map((row) => this.mapEnrichedTimelineEvent(row));
    }
    async getTimelineStats(input) {
    ...
    async getEventDetails(eventId, organizationId) {
    ...
    async cleanupOldEvents(retentionDays = 90, minImportance = 0.3) {
    ...
    async logAgentRun(agentName, executionId, campaignId, organizationId, options) {
    ...
    async logFollowupSent(followupId, sequenceId, stepNumber, contactId, contactEmail, subject, campaignId, organizationId, messageId) {
        return this.logEvent({
            campaignId,
    ...
    async logReviewSubmitted(reviewId, reviewType, priority, title, agentName, campaignId, organizationId, assignedTo) {
        return this.logEvent({
            campaignId,
    ...
    async logDecisionMade(reviewId, decision, userId, userName, feedback, campaignId, organizationId) {
        return this.logEvent({
            campaignId,
    ...
    async logCRMInteraction(interactionId, interactionType, channel, contactId, contactName, subject, campaignId, organizationId, userId, userName) {
        return this.logEvent({
            campaignId,
    ...
    async logTaskExecuted(taskId, taskType, agentName, status, durationMs, campaignId, organizationId, output, error) {
        return this.logEvent({
            campaignId,
...
exports.timelineEngine = new TimelineEngine();
